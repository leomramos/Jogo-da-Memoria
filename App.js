...
`;
  ...
  return (
    <Content
      ...
      disabled={waiting || discovered || chosen || !playing}
      onPress={action}
    />
  );
};
...
export default function App() {
  ...
  const checkArray = (el, item) => {
    return (
      el.findIndex(el => JSON.stringify(el) === JSON.stringify(item)) !== -1
    );
  };
  ...
  const botPlay = () => {
    ...
    if (getRandom(0, 100) <= botUseLast) {
      if (
        !lastPlayed.every(el => {
          ...
          const result = botMemory.find(
            mem => mem.color === color && !(mem.x === el.x && mem.y === el.y)
          );
    ...
    if (getRandom(0, 100) <= botRememberBoth) {
      if (
        !botMemory.every(el => {
          el.color;
          const result = botMemory.find(
            mem => mem.color === el.color && !(mem.x === el.x && mem.y === el.y)
          );
    ...
    while (
      (x === null && y === null) ||
      (checkArray(lastPlayed, { x, y }) &&
        gameColors[lastPlayed[0].x][lastPlayed[0].y].color ===
          gameColors[lastPlayed[1].x][lastPlayed[1].y].color) ||
    ...
    if (getRandom(0, 100) <= botRemember) {
      ...
      const result = botMemory.find(
        mem => mem.color === color && !(mem.x === x && mem.y === y)
      );
  ...
  const checkPoint = _ => {
    ...
    if (gameColors[x][y].color === gameColors[x1][y1].color) {
      ...
      botMemory = botMemory.filter(
        mem => (mem.x !== x || mem.y !== y) && (mem.x !== x1 || mem.y !== y1)
      );
      nextTurn(false);
    } else {
  ...
  return (
    <Container>
      <View style={{ alignItems: "center", marginBottom: 50 }}>
        ...
        />
        <Button
          title={winner !== null ? "Jogar novamente" : "Reiniciar"}
      ...
      <TurnInfo>
        {winner ? winner : curTurn ? "Você joga" : "Máquina joga"}
      </TurnInfo>
      <PointsDisplay>
